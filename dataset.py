...
from datasets.ravdess import DAiSEE

def get_training_set(opt, spatial_transform=None, audio_transform=None):
    assert opt.dataset in ['RAVDESS', 'DAiSEE'], print('Unsupported dataset: {}'.format(opt.dataset))
    ...
    elif opt.dataset == 'DAiSEE':
        training_data = DAiSEE(
            opt.annotation_path,
            'training',
            spatial_transform=spatial_transform, data_type='audiovisual', audio_transform=audio_transform)
