...
# 对视频进行人脸的标注处理，这里并不属于最后的标注操作
def mark_frame_with_heatmap_by_index(im, faces, max_indexs):
    # 热力图值
    scores = np.zeros((im.shape[0], im.shape[1]), dtype=np.uint8)
    for i, box in enumerate(faces):
        ...
        x1, y1 = x_min, y_min
        x2, y2 = x_max, y_max
        circle0 = int(abs(x1 + (x2 - x1) / 2))
        circle1 = int(abs(y1 + (y2 - y1) / 2))
        circle_radius = max(abs(x2 - x1), abs(y2 - y1))

        # 通过index确定热力图的大小
        colors = [0, 1, 0, 0]

        # 在图像上绘制矩形
        cv2.circle(scores, (circle0, circle1), circle_radius, colors[int(max_indexs[i])], -1)

    heatmapshow = None
    heatmapshow = cv2.normalize(scores, heatmapshow, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    heatmapshow = cv2.applyColorMap(heatmapshow, cv2.COLORMAP_JET)
    heatmapshow = cv2.applyColorMap(np.uint8(scores * 255), cv2.COLORMAP_JET)

    # 将映射后的颜色图像与原始图像进行叠加
    alpha = 0.5
    im = cv2.addWeighted(im, alpha, heatmapshow, 1 - alpha, 0)
...
def mark_faces_by_index(im, faces, max_indexs):
    for i, box in enumerate(faces):
        ...
        # 通过index确定人脸框的颜色
        colors = [(0, 0, 255), (0, 255, 255), (0, 255, 0), (170, 85, 255)]

        # 在图像上绘制矩形
        cv2.rectangle(im, (x1, y1), (x2, y2), colors[int(max_indexs[i])], 2)
...
parent = '/home/ubuntu/Videos/教学视频/new_example'
...
for foldername in os.listdir(parent):
    if foldername.endswith('.mp4') or \
       ...
       foldername.endswith('_csv'):
        continue
    target_folder_path = os.path.join(parent, foldername) + '_v2_csv'
    ...
    for file_path in os.listdir(os.path.join(parent, foldername)):
        ...
        if filename.endswith('.mp4'):
            ...
            header = ['begin_stamp', 
                    ...
                    'students_cnt', 'head_up_percentage', 
                    'average_engage', 'average_round_engage']
            with open(os.path.join(target_folder_path, file_path[:-4] + '_get_date.csv'), 'w', newline='') as f:
                ...
                while frame_cnt < total_frames:
                    ...
                    if check_data_already(numpy_video_dataset, frames_per_interval):
                        ...
                        if students_cnt == 0:
                            students_cnt = -1
                        average_engage = sum(engagements) / students_cnt
                        if isinstance(average_engage, torch.Tensor):
                            average_engage = average_engage.item()
                        engage_0, engage_1, engage_2, engage_3 = get_engage(max_indexs)
                        head_up_percentage = head_up / len(head_pose_list) * 100
                        average_round_engage = (engage_1 + engage_2 * 2 + engage_3 * 3) / students_cnt
                        ...
                        if len(face_locations) == len(max_indexs) and save_avi:
                            ...
                            for i in range(detect_faces_interval * math.ceil(fps)):
                                ...
                                # im = mark_frame_with_heatmap_by_index(im, face_locations, max_indexs)
                                out.write(im)
